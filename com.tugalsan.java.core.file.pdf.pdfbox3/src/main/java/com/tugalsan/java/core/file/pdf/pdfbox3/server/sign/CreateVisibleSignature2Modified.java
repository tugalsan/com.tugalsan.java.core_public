...
public class CreateVisibleSignature2Modified extends CreateSignatureBaseModified {
     ...
     * @throws IOException
     */
//    public void signPDF(File inputFile, File signedFile, java.awt.geom.Rectangle2D humanRect, String tsaUrl, String name, String place, String reason) throws IOException {
//        this.signPDF(inputFile, signedFile, humanRect, tsaUrl, null, name, place, reason);
//    }
    /**
     * Sign pdf file and create new file that ends with "_signed.pdf".
     *
     ...
     * @throws IOException
     */
    public void signPDF(File inputFile, File signedFile, java.awt.geom.Rectangle2D humanRect, String tsaUrl, String signatureFieldName, String name, String place, String reason) throws IOException {
        ...
        try (var fos = new FileOutputStream(signedFile); PDDocument doc = Loader.loadPDF(inputFile)) {
            // call SigUtilsModified.checkCrossReferenceTable(doc) if Adobe complains
            // and read https://stackoverflow.com/a/71293901/535646
            // and https://issues.apache.org/jira/browse/PDFBOX-5382

            int accessPermissions = SigUtilsModified.getMDPPermission(doc);
            ...
            if (doc.getVersion() >= 1.5f && accessPermissions == 0) {
                SigUtilsModified.setMDPPermission(doc, signature, 2);
            }
            ...
            if (isExternalSigning()) {
                var externalSigning = doc.saveIncrementalForExternalSigning(fos);
    ...
    private InputStream createVisualSignatureTemplate(PDDocument srcDoc, int pageNum,
        ...
        try (var doc = new PDDocument()) {
            var page = new PDPage(srcDoc.getPage(pageNum).getMediaBox());
            doc.addPage(page);
            var acroForm = new PDAcroForm(doc);
            doc.getDocumentCatalog().setAcroForm(acroForm);
            var signatureField = new PDSignatureField(acroForm);
            var widget = signatureField.getWidgets().get(0);
            ...
            var stream = new PDStream(doc);
            var form = new PDFormXObject(stream);
            var res = new PDResources();
            form.setResources(res);
            form.setFormType(1);
            var bbox = new PDRectangle(rect.getWidth(), rect.getHeight());
            ...
            var appearanceStream = new PDAppearanceStream(form.getCOSObject());
            ...
            try (var cs = new PDPageContentStream(doc, appearanceStream)) {
                ...
                if (imageFile != null) {
                    ...
                    var img = PDImageXObject.createFromFileByExtension(imageFile, doc);
    ...
    private PDSignature findExistingSignature(PDAcroForm acroForm, String sigFieldName) {
        PDSignature signature = null;
        if (acroForm != null) {
            var signatureField = (PDSignatureField) acroForm.getField(sigFieldName);
