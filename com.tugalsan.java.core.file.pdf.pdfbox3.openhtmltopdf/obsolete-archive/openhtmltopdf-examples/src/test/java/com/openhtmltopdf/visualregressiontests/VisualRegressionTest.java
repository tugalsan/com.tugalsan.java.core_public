...
public class VisualRegressionTest {
    ...
    /**
     * Tests fixed elements are repeated on each page and top/right/bottom/left
     * properties for fixed position block elements.
     */
    @Test
    ...
    /**
     * Tests box-sizing: content-box for static block elements. Includes max/min
     * width properties.
     */
    @Test
    ...
    /**
     * Tests box-sizing: border-box for static block elements. Includes max/min
     * width properties.
     */
    @Test
    ...
    /**
     * Tests min/max/height properties with static block elements. Includes both
     * border-box and content-box sizing.
     */
    @Test
    ...
    /**
     * Tests overflow:hidden. Containers are static blocks. Overflow content
     * includes static blocks and floats.
     */
    @Test
    ...
    /**
     * Tests that non-paginated table columns overflow onto inserted shadow
     * page.
     */
    @Test
    ...
    /**
     * Tests that paginated table columns (including header and footer) overflow
     * onto inserted shadow page.
     */
    @Test
    ...
    /**
     * Tests that overflow:hidden content does NOT generate shadow pages.
     * Includes case where content is absolute block and a case where content is
     * a static block.
     */
    @Test
    ...
    /**
     * Tests that overflow hidden clipping works on generated shadow page.
     * Includes case where content is absolute block and a case where content is
     * a static block.
     */
    @Test
    ...
    /**
     * Tests that static block content dows not overflow a static block with
     * overflow:hidden.
     */
    @Test
    ...
    /**
     * Tests that static inline-block content does not overflow a static block
    ...
    /**
     * Tests that a floated block does not overflow a static block with
     * overflow:hidden.
     */
    @Test
    ...
    /**
     * Tests that static block content does not overflow a floater with overflow
     * set to hidden. Similar to {@link #testHiddenBorder()}
     */
    @Test
    ...
    /**
     * Tests that in the case static block :: static block :: static floated
     * block, that the floated element does not overflow its grand parent which
     * has overflow:hidden.
     */
    @Test
    ...
    /**
     * Tests that transformed static blocks do not overflow static block parent
     * with overflow:hidden.
     */
    @Test
    ...
    /**
     * Tests that absolute block does not overflow relative block parent with
     * overflow:hidden. Issue#273.
     */
    @Test
    ...
    /**
     * Tests that static inline-blocks too long for the line stack one on top of
     * the other.
     */
    @Test
    ...
    /**
     * Tests that static inline-blocks that can stack from left to right, top to
     * bottom.
     */
    @Test
    ...
    /**
     * Tests that page-break-inside: avoid works for static inline-block
     * elements.
     */
    @Test
    ...
    /**
     * Tests that page-break-inside: auto works for static inline-block
     * elements.
     */
    ...
    /**
     * With a static block, rotate, large page margin, no block margin or
     * padding.
     */
    @Test
    ...
    /**
     * With static blocks, rotate then translate, no page margin, no block
     * margin or padding.
     */
    @Test
    ...
    /**
     * Tests that a transformed element with fixed position works on multiple
     * pages.
     */
    @Test
    ...
    /**
     * With single static block, rotate then translate, small page margin, no
     * block margin or padding.
     */
    @Test
    ...
    /**
     * With a static block, rotate, large page margin, small block margin, no
     * padding.
     */
    @Test
    ...
    /**
     * With a static block, rotate, large page margin, no block margin, small
     * padding.
     */
    @Test
    ...
    /**
     * With an absolute block, rotate, large page margin, small block margin,
     * small padding, small border.
     */
    @Test
    ...
    /**
     * With a floated static block, rotate, large page margin, small block
     * margin, small padding, small border.
     */
    @Test
    ...
    /**
     * Tests that a transformed element does not impact subsequent elements.
     * Issue 260.
     */
    @Test
    ...
    /**
     * Tests that a grandchild inline-block does not overflow its grandparent
     * with overflow hidden.
     */
    @Test
    ...
    /**
     * Tests that a grandchild absolute does not overflow its grandparent with
     * overflow hidden.
     */
    @Test
    ...
    /**
     * Tests that a grandchild absolute overflows its grandparent with overflow
     * hidden, when the grandparent is not the containing block.
     */
    @Test
    ...
    /**
     * Tests that a grandchild fixed overflows its grandparent with overflow
     * hidden, because the grandparent is not the containing block, the viewport
     * is, even though the grandparent has position relative.
     */
    @Test
    ...
    /**
     * Tests a simple transform in top-center, bottom-center, left-middle,
     * right-middle.
     */
    @Test
    ...
    /**
     * Tests a long text transform in left-middle. Issue no. 269. Common case of
     * wanting a strip of vertical text in the left margin.
     */
    @Test
    ...
    /**
     * Tests a running div with overflow hidden containing a larger replaced
     * img.
     */
    @Test
    ...
    /**
     * Tests that transforms to the left of the page margin generate an overflow
     * page.
     */
    @Test
    ...
    /**
     * Tests that tds with a background-color do not have fine lines between
     * them. Issues: 291 and 169.
     */
    @Test
    ...
    /**
     * Tests than an inline block, floated, with border, that the border is
     * visible. Issue 297.
     */
    @Test
    ...
    /**
     * Tests that an img (with percentage max-width) shows up in an auto width
     * table cell. Issue 313.
     */
    @Test
    ...
    /**
     * 1. Tests that an img (with percentage max-width) shows up in an absolute
     * width table cell. 2. Tests that an img (with absolute max-width) shows up
     * correctly sized in an auto width table cell.
     */
    @Test
    ...
    /**
     * Tests that large images with smaller max-width and/or max-height are
     * correctly resized. PR number 373.
     */
    @Test
    ...
    /**
     * Tests that an image with width and height specified but max-width or
     * max-height takes precedence, that the image will keep its aspect ratio.
    ...
    /**
     * Tests that a fixed position element correctly resizes to the sum of its
     * child boxes using border-box sizing.
     */
    @Test
    ...
    /**
     * Tests that a SVG in a wrapper with no other content and on a named page
     * will not crash. Issue 328.
     */
    @Test
    ...
    /**
     * Tests that styles applying to internal SVG objects such as rect are
     * passed to Batik for rendering.
     */
    @Test
    ...
    /**
     * Tests that styles applying to internal svg objects such as circle in a
     * linked img tag are applied and passed to Batik for rendering.
     */
    @Test
    ...
    /**
     * Tests that a svg image linked from an image element containing unit sizes
     * (px, cm, etc) in the width and height attribute is correctly sized.
     */
    @Test
    ...
    /**
     * Tests that a broken image inside a table cell renders with a zero sized
     * image rather than crashing with a NPE. See issue 336.
     */
    @Test
    ...
    /**
     * Tests that a broken image inside an inline block does not show and does
     * not crash. See issue 336.
     */
    @Test
    ...
    /**
     * Tests various sizing properties for replaced SVG images including
     * box-sizing, min/max, etc.
     */
    @Test
    ...
    /**
     * Tests that in the default secure mode, SVG images will not run script or
    ...
    /**
     * Tests that in insecure mode, the svg renderer will allow scripts and
     * external resource requests.
     *
     * NOTE: This tests downloads
     * <code>https://openhtmltopdf.com/flyingsaucer.png</code> and so will be
     ...
     * successfully renders. Also make sure a missing SVG resource does not
     * shutdown rendering altogether. Issue 353.
     */
    @Test
    ...
    /**
     * Tests that we correctly render PDF pages in the img tag at the correct
     * CSS specified sizing. Issue 344.
     */
    @Test
    ...
    /**
     * Check counter style after page break, it should not be affected by a
     * previous bolded text, see issue
     ...
     * on a page. https://github.com/danfickle/openhtmltopdf/issues/399
     */
    @Test
    ...
    /**
     * Tests that justified text with non-justified content (br) nested inside
     * it will not throw a NPE.
    ...
    /**
     * Tests that in break-word mode, a long word will only be broken if it
     * would not fit on a line by itself.
    ...
    /**
     * Tests additional problems with break-word such as the first word being
     * too long, etc.
     */
    @Test
    ...
    /**
     * Tests the behavior of very large unbreakable characters with break-word
     * enabled.
     */
    @Test
    ...
    /**
     * Tests that the rem unit work with a document that has no author font-size
     * set on the html element (defaults to 16px).
     */
    @Test
     ...
     * circumstances.
     *
     * Turns out this bug was triggered by zero width table cells with content
     * and word-wrap: break-word. Therefore, this test also exercises zero width
     * divs with content (and without).
     */
    @Test
    ...
    /**
     * Tests that position: fixed on multiple pages is correctly positoned on
     * pages after page one.
     */
    @Test
    ...
    /**
     * Tests that aligned right text doesn't have trailing spaces that cause
     * ragged text on the right.
     */
    @Test
    ...
    /**
     * Tests that a google font import will work (provided that truetype font is
     * included).
     */
    @Test
    ...
    /**
     * Tests another endless loop bug when the font reports a non-zero width for
     * soft hyphen and this overflows the width of the box in break-word mode.
     */
    @Test
    public void testIssue482InfiniteLoopTable() throws IOException {
        assertTrue(vt.runTest("issue-482-infinite-loop-table", builder -> {
            builder.useFont(() -> getClassLoader(VisualRegressionTest.class).getResourceAsStream("org/apache/pdfbox/resources/ttf/LiberationSans-Regular.ttf"),
                    "Liberation Sans");
        }));
    }

    /**
     * Tests that too long words fall below floats (left, right, both). This
     * test is needed to make sure fixes for issue 482 do not prevent this
     * behavior.
     */
    @Test
    ...
    /**
     * Ensure there is no NPE exception launched if the decoding of an image
     * fail (base64 case).
     *
    ...
    /**
     * Tests auto page break before line-box with content much larger than
     * line-height.
     */
    @Test
    ...
    /**
     * Tests no infinite loop with this input and that floats with a margin are
     * being cleared properly (the root cause)
     */
    @Test
    ...
    /**
     * Transforms on inline-block or inline elements output twice, once as the
     * non-transformed output and once as transformed. Also affected
     * inline-blocks with a z-index.
    ...
    /**
     * Combinations of inline-blocks with other display and positioned content.
     */
    @Test
    ...
    /**
     * Test for a bug where a table with <code>position: absolute</code> caused
     * an exception as no block formatting context was created for positioned
     * tables.
     */
    @Test
    ...
    /**
     * Tests a now-fixed near-infinite loop when page-break-inside: avoid is
     * used on heavily nested content.
     */
    @Test
    ...
    /**
     * Test weirdness in table borders when PDF is zoomed. Apparently was caused
     * by anti-aliasing selectively applied to beveled borders. To fix we add
     * the <code>-fs-border-rendering</code> property to selectively turn off
     * border bevelling.
     */
    @Test
    ...
    /**
     * Tests the leader CSS function when the text exceeds the width of the
     * page.
     */
    @Test
