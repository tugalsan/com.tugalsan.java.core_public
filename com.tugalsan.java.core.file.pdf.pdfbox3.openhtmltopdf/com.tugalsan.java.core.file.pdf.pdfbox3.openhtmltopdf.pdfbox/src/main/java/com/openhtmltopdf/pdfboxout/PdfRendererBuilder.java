...
public class PdfRendererBuilder extends BaseRendererBuilder<PdfRendererBuilder, PdfRendererBuilderState> {
    ...
    /**
     * An output stream to output the resulting PDF. The caller is required to
     * close the output stream after calling run.
     */
    ...
    /**
     * Set the PDF version, typically we use 1.7. If you set a lower version, it
     * is your responsibility to make sure no more recent PDF features are used.
     */
     ...
     *
     * Note: PDF/A documents require fonts to be embedded. So if this is not set
     * to NONE, the built-in fonts will not be available and currently any text
     * without a specified and embedded font will cause the renderer to crash
     * with an exception.
     */
     ...
     * <code>PDDocument</code>. The user can use this method to create a
     * document either entirely on-disk or a mix of in-memory and on-disk using
     * the <code>PDDocument</code> constructor that takes a
     * <code>MemoryUsageSetting</code>.
     *
    ...
    /**
     * Like
     * {@link BaseRendererBuilder#useFont(FSSupplier, String, Integer, FontStyle, boolean)}
     * but allows to supply a PDFont directly. Subclass {@link PDFontSupplier}
     ...
     *
     * @param producer the name of the producer to set defaults to
     * openhtmltopdf.com
     * @return this for method chaining
     */
    ...
    public enum CacheStore {
        ...
        /**
         * Caches font metrics, based on a combined key of family name, weight
         * and style. Using this cache avoids loading fallback fonts if the
         * metrics are already in the cache and the previous fonts contain the
         * needed characters.
         */
        PDF_FONT_METRICS;
    }
    ...
    /**
     * Use a specific cache. Cache values should be thread safe, so provided
     * your cache store itself is thread safe can be used accross threads.
     *
     * @return this for method chaining.
     * @see CacheStore
     */
