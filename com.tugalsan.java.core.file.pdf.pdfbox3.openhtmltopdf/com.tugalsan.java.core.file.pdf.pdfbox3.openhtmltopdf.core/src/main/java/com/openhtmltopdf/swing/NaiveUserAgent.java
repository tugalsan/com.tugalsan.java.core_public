...
/**
 * <p>
 * NaiveUserAgent is a simple implementation of {@link UserAgentCallback} which
 * places no restrictions on what XML, CSS or images are loaded.</p>
 *
 * <p>
 * The NaiveUserAgent has a simple per-run cache for images so that the same
 * image is not embedded in a document multiple times.</p>
 *
...
public abstract class NaiveUserAgent implements UserAgentCallback, DocumentListener {
    ...
    public static class DefaultHttpStreamFactory implements FSStreamFactory {
         ...
         * Uses URLConnection to perform requests. Zero value for timeout
        ...
        /**
         * Create a FSStreamFactory with 10 second connect timeout and 30 second
         * read timeout.
         */
    ...
    protected InputStream openStream(String uri) {
        ...
        try {
            ...
            if (hasProtocolFactory(protocol)) {
                ...
                try {
                    is = new URI(uri).toURL().openConnection().getInputStream();
    ...
    /**
     * Retrieves the CSS located at the given URI. It's assumed the URI does
     * point to a CSS file--the URI will be resolved, accessed (using the set
     * FSStreamFactory or URL::openStream), opened, read and then passed into
     * the CSS parser. The result is packed up into an CSSResource for later
     * consumption.
     *
    ...
    /**
     * Retrieves the XML located at the given URI. It's assumed the URI does
     * point to a XML--the URI will be accessed (using the set HttpStreamFactory
     * or URL::openStream), opened, read and then passed into the XML parser
     * (XMLReader) configured for Flying Saucer. The result is packed up into an
     * XMLResource for later consumption.
     *
    ...
    /**
     * Returns true if the given URI was visited, meaning it was requested at
     * some point since initialization.
     *
