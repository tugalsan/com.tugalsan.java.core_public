...
/**
 * <p>
 * FSCatalog loads an XML catalog file to read mappings of public IDs for XML
 * schemas/dtds, to resolve those mappings to a local store for the schemas. The
 * catalog file allows one to have a single mapping of schema IDs to local
 ...
 * mapping of public id to local URI using the public element in the catalog
 * XML.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * To use, instantiate the class, and call {@link #parseCatalog(InputSource)} to
 * retrieve a {@link java.util.Map} keyed by public ids. The class uses an
 * XMLReader instance retrieved via {@link XMLResource#newXMLReader()}, so
 * XMLReader configuration (and specification) follows that of the standard XML
 * parsing in Flying Saucer.
 * </p>
 * <p>
 * This class is not safe for multi-threaded access.</p>
 *
...
public class FSCatalog {
    ...
    /**
     * Parses an XML catalog file and returns a Map of public ids to local URIs
     * read from the catalog. Only the catalog public elements are parsed.
     *
     ...
     * read from the catalog. Only the catalog public elements are parsed.
     *
     * @param inputSource A SAX InputSource to a catalog XML file on the
     * classpath.
     */
     ...
     * public IDs to local URIs. Currently only handles the <public> element and
     * attributes. To use, just call XMLReader.setContentHandler() with an
     * instance of the class, parse, then call getEntityMap().
     */
    private static class CatalogContentHandler extends DefaultHandler {
        ...
        /**
         * Receive notification of the beginning of an element; here used to
         * pick up the mappings for public IDs to local URIs in the catalog.
         */
        @Override
    ...
    /**
     * Attempts to set requested feature on the parser; logs exception if not
