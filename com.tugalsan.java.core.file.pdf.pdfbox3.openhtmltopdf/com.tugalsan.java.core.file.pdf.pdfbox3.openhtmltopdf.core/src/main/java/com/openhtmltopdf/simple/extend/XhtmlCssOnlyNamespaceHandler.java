...
/**
 * Handles xhtml but only css styling is honored, no presentational html
 * attributes (see css 2.1 spec, 6.4.4)
 */
public class XhtmlCssOnlyNamespaceHandler extends NoNamespaceHandler {
     ...
     * or the empty string. Caches value so can be called multiple times without
     * performance issues.
     */
    ...
    /**
     * Gets the language of an element as specified (in order of precedence) by
     * the lang attribute on the element itself, the first ancestor with a lang
     * attribute, the Content-Language meta tag or the empty string.
     */
    @Override
