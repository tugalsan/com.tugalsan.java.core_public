...
@Deprecated //TODO
public class TGC_LabelFor extends ButtonBase implements HasName, HasValue<String>, HasChangeHandlers {
     ...
     */
    public TGC_LabelFor(String label) {
        this();
        setText(label);
    }
     ...
     */
    public TGC_LabelFor(String label, boolean asHTML) {
        this();
        if (asHTML) {
            setHTML(label);
        } else {
    ...
    protected TGC_LabelFor(Element elem) {
        ...
        if (!valueChangeHandlerInitialized) {
            addChangeHandler(new ChangeHandler() {
                public void onChange(ChangeEvent event) {
                    ValueChangeEvent.fire(TGC_LabelFor.this, getValue());
                }
            });
     ...
     * submitted.
     * <p>
     * This will probably return the same thing as {@link #getValue}, left here
     * for magic reasons.
     */
     ...
     * <p>
     * @return the value of the input box. Will not return null
     */
     ...
     * is the value that will be associated with the InputLabel's name and
     * submitted to the server if a {@link FormPanel} that holds it is
     * submitted.
     * <p>
     ...
     * <p>
     * Note that this <em>does not</em> set the value property of the input
     * element wrapped by this widget. For access to that property, see
     ...
     * document. onAttach needs special handling for the InputLabel case. Must
